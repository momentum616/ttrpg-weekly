"""
build_index.py
Scans /reports for HTML files and regenerates index.html in the
Dungeon Economy design system. Triggered automatically by GitHub
Actions when a new report is pushed.
"""
...
def extract_meta(path):
    ...
    # Best headline: first h2 or h3 with meaningful length
    ...
    # Preview: first paragraph with enough text
...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    header.hero {{ padding: 42px 0 26px; }}
    .hero-grid {{ display: grid; grid-template-columns: 1.4fr .85fr; gap: 24px; }}
    ...
    .kicker {{
      ...
      color: var(--muted); text-transform: uppercase; letter-spacing: 0.16em; font-size: 0.72rem; font-weight: 800;
    }}
    .kicker::before {{ content: ""; width: 18px; height: 1px; background: var(--accent); display: inline-block; }}
    h1 {{ margin: 0; font-size: clamp(2.25rem, 5vw, 4rem); line-height: 0.96; letter-spacing: -0.045em; }}
    ...
    .chip {{ display: inline-flex; align-items: center; padding: 8px 12px; border-radius: 999px; border: 1px solid var(--line); background: var(--card); color: var(--muted); font-size: 0.9rem; }}
    .header-side {{ padding: 22px; display: flex; flex-direction: column; justify-content: space-between; gap: 18px; }}
    .header-side h2 {{ margin: 0 0 12px; font-size: 0.86rem; text-transform: uppercase; letter-spacing: 0.15em; color: var(--soft); }}
    .header-note {{ margin: 0; font-size: 1rem; }}
    .header-meta {{ border-top: 1px solid var(--line); padding-top: 14px; color: var(--muted); font-size: 0.94rem; }}
    .stats-row {{ display: grid; grid-template-columns: repeat(3, minmax(0,1fr)); gap: 16px; margin: 28px 0 0; }}
    .stat-card {{
      background: linear-gradient(180deg, color-mix(in srgb, var(--bg-elev) 94%, transparent), var(--bg-elev));
    ...
    .section-head h2 {{ margin: 0; font-size: clamp(1.38rem, 2vw, 1.78rem); line-height: 1.12; letter-spacing: -0.02em; }}
    ...
    footer {{ margin-top: 32px; padding: 28px 0 44px; border-top: 1px solid var(--line); color: var(--muted); }}
    .footer-grid {{ display: flex; flex-wrap: wrap; justify-content: space-between; gap: 16px; align-items: end; }}
    .footer-grid strong {{ display: block; color: var(--text); margin-bottom: 4px; }}
    ...
    @media (max-width: 760px) {{
      ...
      .hero-grid, .archive-grid, .stats-row {{ grid-template-columns: 1fr; }}
...
<body>
  <div class="topbar">
    <div class="wrap topbar-inner">
      ...
      <button class="toggle" id="themeToggle" type="button" aria-label="Toggle light and dark mode">Light mode</button>
    </div>
  </div>
    ...
    <div class="wrap hero-grid">
      ...
      <aside class="header-side">
        <div>
          ...
          <p class="header-note">Each issue tracks what changed in D&amp;D and the wider tabletop market, separates confirmed fact from interpretation, and surfaces the commercial and community signals that matter for players, DMs, and creators.</p>
        </div>
